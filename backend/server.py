...
if not JWT_SECRET:
    # Safer: fail-fast in production; allow dev with a warning.
    if is_production():
...
async def submit_contact_form(form_data: ContactFormSubmit):
    ...
    if EMAIL_ENABLED:
        # Note: Resend requires a verified "from" address/domain. If SENDER_EMAIL isn't verified,
        # send will fail and you'll see email_status="failed" with logs.
        html_content = f"""
        <html>
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            ...
            <p style="background:#f4f4f4;padding:15px;border-radius:6px;">{form_data.message}</p>
        ...
        params = {
            ...
            "html": html_content,
            # Resend accepts reply_to; leaving as a string is OK with the Python SDK.
        ...
        try:
            ...
            logger.info(
                "Contact email sent. From=%s To=%s Patient=%s",
                SENDER_EMAIL,
                CONTACT_TO_EMAIL,
                form_data.email,
            )
